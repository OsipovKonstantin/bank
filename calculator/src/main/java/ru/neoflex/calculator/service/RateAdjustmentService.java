...
import ru.neoflex.calculator.model.enums.Gender;
import ru.neoflex.calculator.model.enums.MaritalStatus;
import ru.neoflex.calculator.model.enums.EmploymentPosition;
import ru.neoflex.calculator.util.DateTimeUtils;
import ru.neoflex.calculator.logging.Logging;

import java.math.BigDecimal;

import static ru.neoflex.calculator.util.MoneyUtils.*;
...
public class RateAdjustmentService {
    ...
    private BigDecimal adjustForPosition(BigDecimal rate, EmploymentPosition position) {
        if (position == EmploymentPosition.MID_MANAGER) {
            rate = rate.subtract(MID_MANAGER_DISCOUNT);
        } else if (position == EmploymentPosition.TOP_MANAGER) {
